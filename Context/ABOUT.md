...
- **AI Roast Generation:** Ollama cloud API (OpenAI-compatible endpoint at `https://ollama.com/v1`), default model `kimi-k2.6:cloud`. In the VS Code extension, configured via settings (`gitgud.ollamaApiKey`, `gitgud.ollamaModel`, `gitgud.ollamaBaseUrl`). Core package accepts `RoastConfig` programmatically.
...
Monorepo fully set up (Stage 1 complete). Git analysis engine built with all analyzers and tests (Stage 2 complete). Full roast/scoring/personality layer complete (Stage 3 complete) — AI roast generation via Ollama cloud API (`kimi-k2.6:cloud`) with brainrot slang library and 70+ template fallbacks, scoring engine, rank system, achievement system (10 achievements), teammate suffering calculator, developer personality classifier (7 archetypes), and unified async `evaluate()` pipeline all implemented with tests (141 passing). VS Code extension MVP complete (Stage 4 complete) — git event detection via VS Code Git API + `.git/` file watchers (`event-detector.ts`, `event-mapper.ts`), notification popups with severity-based routing (`roast-notifier.ts`), local persistence via `globalState` with schema versioning (`state-manager.ts`, `migration.ts`), full pipeline wired in `extension.ts` (detect → analyze → evaluate → notify → persist → update sidebar), sidebar webview dashboard (`sidebar-provider.ts`) with rank display, score progress bar, teammate suffering meter, personality classification, recent offenses feed, achievement grid with progress, and stat summary — registered as activity bar panel. Centralized configuration (`config.ts`) with `onDidChangeConfiguration` listener. Sound effects system (`sound-player.ts`) using Web Audio API synthesis in the webview — rank up/down jingles, achievement fanfare, critical alert, event blip — no external audio files required. Five commands registered (`gitgud.showStatus`, `gitgud.resetStats`, `gitgud.showProfile`, `gitgud.showDashboard`, `gitgud.toggleSound`), and settings for enable/disable, notifications toggle, sound toggle, roast intensity (`mild`/`medium`/`savage`), and Ollama API configuration. Next.js 16 web package scaffolded. Stages 5–6 not yet started.
