...
function test(event, label) {
    ...
    if (result.analysis.verdicts.length) {
        console.log('📋 Verdicts:');
        result.analysis.verdicts.forEach(v => {
            console.log(`   [${v.severity.toUpperCase()}] (${v.category}) ${v.message}`);
        });
    }
}

// 1. Lazy commit on main
test({
    type: 'commit',
    timestamp: Date.now(),
...
console.log('\n========================================');
console.log('All roasts generated! Scroll up to see.');
console.log('========================================');
