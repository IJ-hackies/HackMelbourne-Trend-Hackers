...
import { GitEventType, GitEventMetadata } from '@git-gud/core';

const pexec = promisify(exec);

type Emit = (type: GitEventType, metadata: GitEventMetadata) => Promise<void> | void;
...
export class MergeConflictTracker {
  ...
  constructor(private repoPath: string, private emit: Emit) {
    this.out = vscode.window.createOutputChannel('Git Gud Conflicts');
  }
  ...
  async start(seedFiles?: string[]) {
    ...
    // Retry-fetch with backoff in case index isn't marked unmerged yet
    if (files.length === 0) {
    ...
    this.out.appendLine(`Tracker started. Conflicted files (${files.length}): ${files.map((f) => path.basename(f)).join(', ') || 'NONE'}`);
    ...
    // Emit a preview event for every detected block so the AI can comment on each upfront
    for (const [absPath, state] of this.files) {
      const relPath = path.relative(this.repoPath, absPath);
      for (const block of state.blocks) {
        await this.emit('conflict_block_preview', {
          filePath: relPath,
          blockIndex: block.id,
  ...
  stop() {
    ...
    this.disposables = [];
    this.out.appendLine('Tracker stopped.');
  }
  ...
  private initFile(absPath: string) {
    try {
      ...
      this.out.appendLine(`  → ${path.basename(absPath)}: ${blocks.length} block(s)`);
    } catch (e) {
      this.out.appendLine(`  → init failed for ${absPath}: ${e}`);
    }
  }
  ...
  private classifyResolution(resolution: string, ours: string, theirs: string): GitEventMetadata['resolutionType'] {
    ...
    const containsOurs = o.length > 0 && r.includes(o);
    const containsTheirs = t.length > 0 && r.includes(t);
    if (containsOurs && containsTheirs) return 'merged_both';
    return 'custom_edit';
  }
  ...
  private async onSave(absPath: string, content: string) {
    ...
    for (const cur of currentBlocks) {
      const match = state.blocks.find(
        (b) => !stillPresentIds.has(b.id) && b.ours === cur.ours && b.theirs === cur.theirs,
      );
    ...
    const relPath = path.relative(this.repoPath, absPath);
    this.out.appendLine(`Save in ${path.basename(absPath)}: resolved ${resolved.length}, remaining ${remaining}`);

    for (const block of resolved) {
      const resolution = this.extractResolution(content, block.anchorBefore, block.anchorAfter);
      const resolutionType = resolution !== null
        ? this.classifyResolution(resolution, block.ours, block.theirs)
        : 'custom_edit';

      await this.emit('conflict_block_resolved', {
        filePath: relPath,
        remainingBlocks: remaining,
        totalBlocks: state.totalBlocks,
        resolutionType,
        resolvedSnippet: resolution ? this.truncate(resolution) : undefined,
        oursSnippet: this.truncate(block.ours),
        theirsSnippet: this.truncate(block.theirs),
        blockIndex: block.id,
      });
    ...
    if (remaining === 0) {
      await this.emit('file_fully_resolved', {
        filePath: relPath,
        totalBlocks: state.totalBlocks,
      });
