...
import { GitEvent, GitEventMetadata, GitEventType } from '@git-gud/core';

type EventHandler = (event: GitEvent) => Promise<void> | void;
...
export class GitWatcher {
  ...
  private async emit(type: GitEventType, metadata: GitEventMetadata = {}) {
    const root = this.workspacePath();
    if (!root) return;
  ...
  private setupWatchers(root: string) {
    ...
    // Watch ALL files inside .git/ — VS Code FS watcher handles creation/change/delete
  ...
  private handleGitFileChange(rel: string, gitDir: string) {
    ...
    if (rel === 'HEAD') {
      ...
      if (newHead && newHead !== this.lastHead) {
        ...
        if (prev !== next) {
          this.emit('branch_switch', { branchName: next, previousBranch: prev });
        }
      }
      return;
    }

    // MERGE_HEAD / rebase dirs
    if (rel === 'MERGE_HEAD' || rel.startsWith('rebase-merge') || rel.startsWith('rebase-apply')) {
      this.checkConflictAndRebaseState(gitDir);
      return;
    }

    // Remote ref logs — push detection
  ...
  private checkConflictAndRebaseState(gitDir: string) {
    ...
    if (mergeExists && !this.mergeConflictActive) {
      ...
      this.emit('merge_conflict_start', { branchName: this.currentBranch() });
    } else if (!mergeExists && this.mergeConflictActive) {
      this.mergeConflictActive = false;
      this.emit('merge_conflict_resolved', { branchName: this.currentBranch() });
    }
    ...
    if (rebaseExists && !this.rebaseActive) {
      ...
      this.emit('rebase_start', { branchName: this.currentBranch() });
    } else if (!rebaseExists && this.rebaseActive) {
      this.rebaseActive = false;
      this.emit('rebase_complete', { branchName: this.currentBranch() });
    }
  }
  ...
  private handleRemoteLogChange(logPath: string, branchName: string) {
    ...
    if (onMain) this.emit('push_to_main', { branchName });

    if (last.includes('forced-update') || last.toLowerCase().includes('force')) {
      this.emit('force_push', { branchName });
    } else if (oldHash && newHash && oldHash !== '0000000000000000000000000000000000000000') {
      // Heuristic: non-fast-forward would need git ancestry check; skipped.
    }
  }
